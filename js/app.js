...
***************************************************************************** */
        ...
        document.addEventListener("click", (function(e) {
            ...
            if (e.target.closest(".btn--next")) {
                ...
                let formRequiredItems = currentCard.querySelectorAll("*[data-required]");
                if (formRequiredItems) formRequiredItems.forEach((formRequiredItem => {
                    if ("" === formRequiredItem.value) {
                        formRequiredItem.classList.add("error");
                        required = false;
                        formRequiredItem.parentElement.setAttribute("style", "position: relative;");
                        if (!formRequiredItem.parentElement.querySelector(".form__error")) formRequiredItem.parentElement.insertAdjacentHTML("beforeend", `<div class="form__error" style="position: absolute; right: 0; bottom: 0; transform: translateY(105%); color: red; font-size: 12px;">required field</div>`);
                        return;
                    } else if (formRequiredItem.classList.contains("error")) {
        ...
        function findElements() {
            script_form = document.querySelector("#form");
            ({action} = script_form);
        }
        ...
        function onSubmit(event) {
            ...
            if (required) {
                ...
                sendForm(currentTarget).then((response => onSuccess(response, currentTarget))).catch(onError);
            }
        }
        function subscribe() {
            script_form.addEventListener("submit", onSubmit);
        }
        function init() {
            findElements();
            subscribe();
